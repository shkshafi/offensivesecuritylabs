  ...
  return (
    <div className="min-h-screen bg-transparent text-slate-700 dark:text-slate-200 font-sans selection:bg-blue-500/30">
      <main className="w-full px-10 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6 pt-1 pb-2 mb-8">
          ...
          <div className="flex items-center gap-3 sm:w-auto justify-end">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-full bg-slate-100 dark:bg-white/[0.02] hover:bg-slate-200 dark:hover:bg-white/[0.06] border border-slate-200 dark:border-white/[0.05] text-sm font-medium transition-all text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-full bg-blue-600 hover:bg-blue-700 text-white font-semibold transition-all shadow-sm shrink-0 border-0 cursor-pointer"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 mb-8">
          ...
          {/* Left Column: Active Reports (col-span-4) */}
          <div className="lg:col-span-4 flex flex-col gap-4">
            ...
            {reports.length === 0 ? (
              <div className="bg-white dark:bg-slate-800 border border-dashed border-slate-200 dark:border-slate-700/50 rounded-2xl p-12 text-center flex flex-col items-center justify-center min-h-[450px] shadow-sm">
                ...
                <button
                  onClick={() => setShowCreateModal(true)}
                  className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-400 text-white dark:text-slate-950 font-bold rounded-full transition-all shadow-[0_0_15px_rgba(59,130,246,0.3)]"
                >
                  Create report
                </button>
              </div>
                    ...
                    >
                      ...
                      <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between border-t sm:border-t-0 sm:border-l border-slate-200 dark:border-slate-700/50 pt-4 sm:pt-0 sm:pl-5 mt-2 sm:mt-0 relative z-10">
                        ...
                        <button
                          ...
                          className="flex items-center gap-2 bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700/50 px-4 py-2 rounded-full text-sm font-medium text-slate-800 dark:text-slate-100 transition-all group-hover:border-slate-300 dark:group-hover:border-slate-600"
                        >
                          Open editor <ArrowRight className="w-4 h-4 text-slate-500 dark:text-slate-400 group-hover:text-slate-800 dark:group-hover:text-white transition-colors" />
                        </button>
                      </div>
                    </div>
                  );
          ...
          {/* Right Column: Stats, Distributions & Recent Activity Stacked (col-span-1) */}
          <div className="lg:col-span-1 flex flex-col gap-6">

            {/* Stats Card: Active Report Count */}
            <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700/50 rounded-xl p-5 relative overflow-hidden group hover:border-slate-300 dark:hover:border-slate-600 transition-colors shadow-sm flex flex-col justify-center min-h-[120px]">
              <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/5 rounded-full blur-2xl -mr-10 -mt-10"></div>
              <div className="flex items-center gap-2 mb-3">
                <FileText className="w-4 h-4 text-slate-400" />
                <span className="text-xs font-semibold uppercase tracking-wider text-slate-500">Active Reports</span>
              </div>
              <span className="text-3xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">{reports.length}</span>
            </div>

            {/* Severity Analytics Chart (Report Status Distribution) */}
            <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700/50 rounded-xl p-5 flex flex-col shadow-sm">
              ...
              {totalReports === 0 ? (
                <div className="h-[100px] flex flex-col items-center justify-center text-slate-500">
                ...
                <div className="flex flex-col gap-4">
                  <div className="h-[140px] w-full relative">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center flex-col pointer-events-none">
                      <span className="text-xl font-bold text-slate-800 dark:text-white">{totalReports}</span>
                      <span className="text-[9px] uppercase font-bold text-slate-500">Reports</span>
                    </div>
                  </div>

                  {/* Custom Legend */}
                  <div className="flex flex-col gap-2 pt-2 border-t border-slate-200 dark:border-slate-700/50">
                    {[
                      ...
                      <div key={item.label} className="flex items-center justify-between">
                        ...
                        <span className="text-[11px] font-bold text-slate-800 dark:text-white">{item.count}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="p-6">
              ...
              <form onSubmit={handleCreate} className="space-y-4">
                ...
                <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-slate-200 dark:border-slate-700/50">
                  ...
                  <button
                    type="submit"
                    className="px-5 py-2.5 bg-slate-900 text-white dark:bg-blue-500 dark:text-slate-950 hover:bg-slate-800 dark:hover:bg-blue-400 text-sm font-bold rounded-full transition-all shadow-md dark:shadow-[0_0_15px_rgba(255,255,255,0.1)]"
                  >
                    Create report
                  </button>
                </div>
